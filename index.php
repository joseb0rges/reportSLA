					...
					<div class="container">
						<div class="form-group">
							...
							<select class="form-control" name="OS">
								<?php 
								for ($i = 0; $i < sizeof($dados->getNameParent()); $i++){ ?>
									<option><?php echo $dados->getNameParent()[$i];?></option>
									<?php 
								}
								?>
							</select>
	...
	<div class="container">
		<div class="principal">
			...
			if(isset($_POST["call01"]) && (!empty($_POST["call01"])) && isset($_POST["call02"]) && (!empty($_POST["call02"])) && isset($_POST["OS"]) && (!empty($_POST["OS"]))){
				...
				<center>
					<h3>
						<strong>RELATÓRIO DE SLA</strong>
					</h3>
				</center><br/>
						...
						<div class="form-group mx-sm-3  mb-2 pull-right">
							...
							<div class="table-wrapper-scroll-y my-custom-scrollbar">
								<table class="table table-striped  table-hover">
									...
									for ($i = 0; $i < sizeof($dependencesID); $i++) {
										...
										if ($dependencesID[$i]->parentDependencies[0]->serviceid == $serviceParentID[0]->serviceid){
											...
											<tbody id="myTable">
												<tr>
													<td align='center'><?php echo utf8_encode($dados->getNameSLA($dependencesID[$i]->serviceid)[0]->name);?>
												</td>
												<td align='center'><?php echo $dados->getSLA($dependencesID[$i]->serviceid,$from,$to);?>
											</td>
											<td align='center'><?php if (substr(100 - $dados->getSLA($dependencesID[$i]->serviceid,$from,$to),0,5) == 0){ ?>
												<span class="alert alert-success" role="alert">NAO HOUVE</span>
												<?php
											}else { 

												?>
												<span class="alert alert-danger" role="alert">
													<?php echo substr(100 - $dados->getSLA($dependencesID[$i]->serviceid,$from,$to),0,5);?></span>
													<?php 
												}
												?>
											</td>
